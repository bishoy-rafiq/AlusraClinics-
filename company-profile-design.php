...
<head>
    ...
    <title>تصميم بروفايل الشركة - إظهار احترافية أعمالك</title>
<meta name="description" content="قم بتصميم بروفايل شركة احترافي يعكس إنجازاتك ويعزز الثقة لدى عملائك وشركائك المحتملين.">
...
<!-- Navbar End -->
     ...
     <!-- section -->
     <?php 
        $get_name_servise10=$db->getData("services","*","id=10")
       ?>  
    ...
    <h2 class="section__header1"><?php echo $get_name_servise10[0]['name']; ?></h2>

        <p>
    <?php echo $get_name_servise10[0]['article']; ?>
        ...
        <img src="./images_site/services/<?php echo $get_name_servise10[0]['image']; ?>" alt="مونتاج وتعديل الفيديوهات" class="w-100 rounded-5" />
        </div>
      </div>
    </div>
    </secttion>
    ...
    <div class="swiper-container1">
        <div class="swiper-wrapper">
            ...
            $imageData = $db->getData("works", "*", "service_id=10 LIMIT 1"); // تحديد خدمة معينة
            if (!empty($imageData)) {
...
<section class="section__container1 gallery__container1">
    <div class="swiper-container1">
        <div class="swiper-wrapper">
            <?php
            // جلب الصور من قاعدة البيانات
            $imageData = $db->getData("works", "*", "service_id=110 LIMIT 1"); // تحديد خدمة معينة
            if (!empty($imageData)) {
                foreach ($imageData as $work) {
                    $images = json_decode($work['images'], true); // فك التشفير للحصول على الصور
                    foreach ($images as $image) {
