...
include_once("../../../../../database/db.php");
   ...
   $db = new db();
    ...
    $service_data = $db->getData("services", "*", "id = $service_id");
    ...
    if (isset($_POST['update_service'])) {
        ...
        $updated_description = $_POST['service_description'];
        ...
        $update_result = $db->update_data("services", $data, $condition);
                                ...
                                <div class="card w-100">
                                    ...
                                    <div class="card-body">
                                        <form action="" method="post">
                                            <div class="mb-3">
                                                <label for="service_name" class="form-label">الاسم</label>
                                                <input type="text" id="service_name" name="service_name" value="<?php echo $service_data[0]['name']; ?>" class="form-control" required/>
                                            </div>
                                            <div class="mb-3">
                                                <label for="service_description" class="form-label">الوصف</label>
                                                <textarea id="service_description" name="service_description" class="form-control" required><?php echo $service_data[0]['description']; ?></textarea>
                                            </div>
                                            <div class="mb-3">
                                                <label for="service_pag" class="form-label">المقال</label>
                                                <textarea id="service_pag" name="service_pag" class="form-control" required><?php echo $service_data[0]['article']; ?></textarea>
                                            </div>
                                            <div class="mb-3">
                                                <label for="category_id" class="form-label">اسم التصنيف</label>
                                                <select id="category_id" name="category_id" class="form-select" required>
                                                    <?php
                                                        $get_categories = $db->get_data("categories");
                                                        foreach ($get_categories as $category) {
                                                            $selected = ($category['id'] == $service_data[0]['category_id']) ? 'selected' : '';
                                                            echo "<option value='{$category['id']}' $selected>{$category['name']}</option>";
                                                        }
                                                    ?>
                                                </select>
                                            </div>
                                            <button type="submit" name="update_service" class="btn btn-success"> تحديث الخدمة</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
