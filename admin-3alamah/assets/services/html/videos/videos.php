...
<body>
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle active" data-bs-toggle="dropdown"><i class="fa fa-video me-2"></i>فيديوهات</a>
                        <div class="dropdown-menu bg-transparent border-0">
                            <a href="add_video.php" class="dropdown-item"> اضافة فيديو</a>
                            <a href="#" class="dropdown-item">قائمة الفيديوهات</a>
                        </div>
                    </div>
        ...
        <div class="content">
            ...
            <?php
                if (isset($_SESSION["error_deleted_video"])) {
                    echo $_SESSION["error_deleted_video"];
                    unset($_SESSION["error_deleted_video"]); 
                }
                if (isset($_SESSION["deleted_video"])) {
                    echo $_SESSION["deleted_video"];
                    unset($_SESSION["deleted_video"]); 
                }
            ?>
                    ...
                    <div class="col-sm-12 col-xl-12">
                        <div class="bg-light rounded p-4">
                            <div class="d-flex justify-content-center mb-1">
                                <a href="add_video.php" class="btn btn-success">اضافة  فيديو</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-sm-12 col-xl-12">
                        <div class="bg-light rounded p-4">
                            <div class="d-flex justify-content-center mb-1">
                                ...
                                ?>
                                <form action='videos.php' class="centered-container">
                                    <select id="service_name" name="service_id" required>
                                        <option selected disabled>اختر الخدمة</option>
                                        <?php echo $services; ?>
                                    </select>
                                    <button type='submit' class="btn btn-success">عرض الفيديوهات</button>
                                </form>                          
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="view_video">
                                <?php
                                    if (isset($_GET['service_id']) && is_numeric($_GET['service_id'])) {
                                        $videosData = $db->getData("services_videos", "urls_videos", "service_id=" . $_GET['service_id']);
                                        if (!empty($videosData)) {
                                            $videos = json_decode($videosData[0]['urls_videos'], true);
                                            // $uniqueVideos = array_unique($videos);
                                            
                                            echo '<div class="row">';
                                            foreach ($videos as $videoId) {
                                                echo '<div class="col-md-6">';
                                                echo '<div class="video-uniqueVideos">';
                                                echo '<iframe src="https://www.youtube.com/embed/' . $videoId . '" frameborder="0" allowfullscreen></iframe>';
                                                echo '<a href="#" class="btn btn-danger delete-btn" onclick="return confirmDelete(\'' . $videoId . '\', ' . $_GET['service_id'] . ')">حذف</a>';
                                                echo '</div>';
                                                echo '</div>';
                                            ...
                                            echo '<div class="card" style="width: 18rem;"><p class="error">لا يوجد فيديوهات</p></div>';
                                        }
                                    }
                                ?>
                            </div>
    ...
    <script src="../../../../LinkNavbar/js/main.js"></script>
     ...
     <script>
        ...
        function confirmDelete(videoId, service_id) {
            console.log('Delete button clicked:', videoId, service_id);
            if (confirm('هل أنت متأكد أنك تريد حذف الفيديو؟')) {
                window.location.href = '../../../delete.php?page=video&video_id=' + encodeURIComponent(videoId) + '&service_id=' + encodeURIComponent(service_id);
            }
            return false;
        }
